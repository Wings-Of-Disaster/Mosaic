...
class ClientTrainer:
    ...
    def train_locally_step(self, I, step, global_params):
        ...
        self.global_model_params = {k: v.clone().detach() for k, v in self.model.state_dict().items()}
        ...
        for tau in range(I):
            ...
            for name, param in self.model.named_parameters():
                ...
                fedprox_reg += ((param - self.global_model_params[name]) ** 2).sum()
            fedprox_reg *= mu / 2
            
            total_loss = loss + fedprox_reg
            total_loss.backward()
